    ...
    def get_face_pixels(self, image: np.ndarray) -> Optional[np.ndarray]:
        ...
        for id in range(0, len(self.LIPS_LANDMARKS) + 1):
            ...
            for i in lips_points:
                if self.LIPS_LANDMARKS[id] == i[0] and first == None:
                    start = first = (i[1], i[2])
        ...
        # face
        start, first, second = None, None, None
        for id in range(0, len(self.FACE_LANDMARKS) + 1):
        ...
        for id in range(0, len(self.RIGHT_EYE_LANDMARKS) + 1):
            ...
            for i in right_eye_points:
                if self.RIGHT_EYE_LANDMARKS[id] == i[0] and first == None:
                    start = first = (i[1], i[2])
        ...
        # left eye
        start, first, second = None, None, None
        for id in range(0, len(self.LEFT_EYE_LANDMARKS) + 1):
            if id == len(self.LEFT_EYE_LANDMARKS):
                cv2.line(elem_mask, first, start, (255, 255, 255), 1)
                break
            for i in left_eye_points:
                if self.LEFT_EYE_LANDMARKS[id] == i[0] and first == None:
                    start = first = (i[1], i[2])
